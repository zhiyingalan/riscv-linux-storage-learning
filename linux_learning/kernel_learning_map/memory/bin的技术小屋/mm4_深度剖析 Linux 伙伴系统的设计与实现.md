...
![img](https://pic1.zhimg.com/v2-c5e3ba777d57b8227abeb2e83635cbda_1440w.jpg)
...
![img](https://pica.zhimg.com/v2-9958ad428c7b5092725a535fc480e9f8_1440w.jpg)
...
![img](https://picx.zhimg.com/v2-ac00687fd49e6ccda406bf8495c9eb9d_1440w.jpg)
...
![img](https://pic1.zhimg.com/v2-4d373e87777395f82abf987965d29298_1440w.png)
...
> 注意：nr_free 表示的可不是空闲内存页 page 的个数，而是**空闲内存块**的个数，对于 0 阶的内存块来说 nr_free 确实表示的是单个内存页 page 的个数，因为 0 阶内存块是由一个 page 组成的，但是对于 1 阶内存块来说，nr_free 则表示的是 2 个 page 集合的个数，以此类推对于 n 阶内存块来说，nr_free 表示的是 2 的 n 次方 page 集合的个数
...
![img](https://pic2.zhimg.com/v2-5828ac51ef85029f5e12b74e9fcad68f_1440w.jpg)
...
![img](https://pic4.zhimg.com/v2-dd8deb2a9505485410194dd9ae18a42f_1440w.jpg)
...
![img](https://picx.zhimg.com/v2-745b6043341025c68d5bdb37568bcab3_1440w.jpg)



我们可以通过 `cat /proc/pagetypeinfo` 命令可以查看当前各个内存区域中的伙伴系统中不同页面迁移类型以及不同 order 尺寸的内存块个数。

![img](https://pic2.zhimg.com/v2-4798ed5572482cfaa58d8f6772ddd9d1_1440w.jpg)
...
![img](https://pic3.zhimg.com/v2-1c3a571e61e03796ddaf7af786a0e306_1440w.png)
...
![img](https://pica.zhimg.com/v2-8f0a7ff8d414ccd9852587a1a06ca15c_1440w.jpg)
...
![img](https://pic4.zhimg.com/v2-f7822501ae495bc87ba3223d2deae645_1440w.jpg)
...
![img](https://pic3.zhimg.com/v2-6d0eb57852f24ea3a07a2668d20d5f38_1440w.jpg)
...
![img](https://pic2.zhimg.com/v2-b96e2c65fc7fc0aa2ddc53297872f3d7_1440w.jpg)
...
以上流程就是伙伴系统的**核心**内存分配过程，下面我们再把内存页面的迁移属性 MIGRATE_TYPES 考虑进来，来看一下完整的伙伴系统内存分配流程：
...
![img](https://pic1.zhimg.com/v2-a8975d083af3db8a980b90890d5aea62_1440w.jpg)
...
![img](https://pic1.zhimg.com/v2-918ee821d20a047a6b7a81ad166cef70_1440w.png)
...
![img](https://pic4.zhimg.com/v2-a4ab46abe8b4d5ffd22e0dd520a49a37_1440w.jpg)
...
![img](https://pic1.zhimg.com/v2-80b7b10c8bba7dfbaa34b3912040b442_1440w.jpg)
...
![img](https://pic1.zhimg.com/v2-13441ff7878888dc4addf5c9ffef9558_1440w.jpg)
...
![img](https://pic3.zhimg.com/v2-5892a389ee694f6d649b6652f519dc10_1440w.jpg)
...
![img](https://picx.zhimg.com/v2-ca7898548551bcaf613569878a1e678f_1440w.jpg)
...
![img](https://pic2.zhimg.com/v2-58ed25d308721653534dcd4ceeaa27e1_1440w.jpg)
...
### **6.2.1 __rmqueue_smallest 伙伴系统的核心实现**
...
从上述 __rmqueue 函数的源码实现中我们可以看出，该函数处理了伙伴系统内存分配的异常流程，即调用 __rmqueue_smallest 进入伙伴系统分配内存时，发现伙伴系统各个分配阶 free_area[order] 中对应的迁移列表 free_list[MIGRATE_TYPE] 无法满足内存分配需求时，__rmqueue_smallest 函数就会返回 null，伙伴系统内存分配失败。
...
> 在系统初始化期间，所有页都被标记为 MIGRATE_MOVABLE 可移动的页面类型，其他的页面迁移类型都是后来通过 __rmqueue_fallback **窃取**产生的。而是否能够窃取 fallback 迁移类型列表中的页面，就是本小节介绍的内容。
...
![img](https://pic1.zhimg.com/v2-d991a97b6a5304da5f5d23065e25ee5a_1440w.jpg)
